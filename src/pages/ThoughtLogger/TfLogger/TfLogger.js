...
class TfLogger extends Component{
  constructor(props){
    super(props);
    console.log(props);
    this.steps = [ "Select Emotion","Select Form","Submit!"];
    this.state = {
      perception:[],
      emotion:[],
      form:[],
  ...
  submitLog=()=>{
    let {perception, form, cypherQuery} = this.state;
    ...
    perception[1].addRelationship("IN_RELATIONSHIP_TO",form[1].variable);
  ...
  getStepContent=(stepIndex)=>{
    switch (stepIndex) {
      case 0:
        return <ManifestedPerception label="Emotion" date={this.state.date} queryKey="perception" submitPerception={this.submitPerception}/>;
      case 1:
  ...
  render(){
    ...
    return(
      <div className={this.state.classList}>
        <div className={"TfLogger-output"}>{this.state.text}</div>
